...
def get_imags():
	...
	# 如果FriendImags目录不存在就创建一个
	if not os.path.exists(curr_dir + "FriendImags/"):
		os.mkdir(curr_dir + "FriendImags/")

	# 登录微信并获取好友信息
	bot = Bot()
	my_friends = bot.friends(update=True)
	# 获取好友头像信息并存储在FriendImags目录中
	n = 0
	for friend in my_friends:
		friend.get_avatar(curr_dir + "FriendImags/" + str(n) + ".jpg")
		n = n + 1
...
def Photo_Wall():
	...
	# 获取下载的头像文件
	curr_dir = get_dir(sys.argv[0])
	ls = os.listdir(curr_dir + 'FriendImags')
